...
def getMenu(request):
    ...
    restaurant_type = request.data
    # 0=교직원식당, 1=학생식당
    if(restaurant_type['restaurant_type'] == 0):
        return Response({"URL": teacher_restaurant}, status=status.HTTP_200_OK)
    else:
        return Response({"URL": student_restaurant}, status=status.HTTP_200_OK)